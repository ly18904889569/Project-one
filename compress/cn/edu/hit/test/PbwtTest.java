...
public class PbwtTest {
	...
	// 写一个测试用例，使得Alignment不是对齐的。
	{
		...
		for (int i = 0; i < 15; i++) {	//liyang:模拟生成read一共15条
			ArrayList<Integer> list = new ArrayList<Integer>();
			list.add(ReadElemEnum.START.ordinal());		//liyang:这个就是将枚举的字母转换为数字 start为2
			for (int j = 0; j < 30 - 2 -10 ; j++) {		//liyang:长度不算上开头和结尾一共长度为18（开头和结尾一共站两个）
...
//		}
	...
	public void PBWTAlgo() {
		 ...
		 */
		ArrayList<Integer> a = new ArrayList<Integer>(); // 存储为0的值		//liyang:这个就是论文中那个存储0的a数组
