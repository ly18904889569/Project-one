	...
	private ArrayList<String> deEncodeQual(byte[] pbwtQual)
	{
		...
		if (pbwtQual.length%3 == 0)
		{
			for(int i=0; i<pbwtQual.length; i+=3)
			{
				...
				deStr[2] = Byte.toString((byte) ((((pbwtQual[i]&0xff)%4)*2)+((((pbwtQual[i+1]+128)&0xff) >> 7)%2)));
				deStr[3] = Byte.toString((byte) (((pbwtQual[i+1]&0xff) >> 4)%8));
				deStr[4] = Byte.toString((byte) (((pbwtQual[i+1]&0xff) >> 1)%8));
				deStr[5] = Byte.toString((byte) ((((pbwtQual[i+1]&0xff)%2)*4)+((((pbwtQual[i+2]+128)&0xff) >> 6)%4)));
...
//				System.out.print(" " + deStr2[0] + " " + deStr2[1] );
			...
			if (pbwtQual.length%3 ==2)
			{
				...
				deStr3[2] = Byte.toString((byte) ((((pbwtQual[pbwtQual.length-2]&0xff)%4)*2)+((((pbwtQual[pbwtQual.length-1]+128)&0xff) >> 7)%2)));
...
//		ArrayList<String> dexQual = new ArrayList<>();
		...
		if (exQual.length%3 == 0)
		{
			for(int i=0; i<exQual.length; i+=3)
			{
				...
				deStr[2] = Byte.toString((byte) ((((exQual[i]&0xff)%4)*2)+((((exQual[i+1]+128)&0xff) >> 7)%2)));
				deStr[3] = Byte.toString((byte) (((exQual[i+1]&0xff) >> 4)%8));
				deStr[4] = Byte.toString((byte) (((exQual[i+1]&0xff) >> 1)%8));
				deStr[5] = Byte.toString((byte) ((((exQual[i+1]&0xff)%2)*4)+((((exQual[i+2]+128)&0xff) >> 6)%4)));
...
//				System.out.print(" " + deStr2[0] + " " + deStr2[1] );
			...
			if (exQual.length%3 ==2)
			{
				...
				deStr3[2] = Byte.toString((byte) ((((exQual[exQual.length-2]&0xff)%4)*2)+((((exQual[exQual.length-1]+128)&0xff) >> 7)%2)));
...
//		ArrayList<ArrayList<Integer>> repbwt = PBWTAlgoRe(pbwtResult, start,end);	// 之前需要其实和终止位置
...
private static int deCompressLen(byte[] len2)
{
	...
	while(i<len2.length && flag)
	{
		...
		{
		case 0:
			num = (len2[0]&0xff)*16777216 + (len2[1]&0xff)*65536 + (len2[2]&0xff)*256 + (len2[3]&0xff);
			break;
		case 1:
...
public static boolean PbwtIsTrue(ArrayList<ArrayList<Integer>> pbwtResult, ArrayList<ArrayList<Integer>> repbwt)
{
	...
	{
		for(int j=0; j<pbwtResult.get(i).size()&&j<pbwtResult.get(i).size();j++)
		{
...
//			ArrayList<ArrayList<String>> exceptionList = new ArrayList<ArrayList<String>>();
//			ArrayList<ArrayList<String>> exceptionListQual = new ArrayList<ArrayList<String>>();
...
//					qualC.add(curValQual);
				...
				{
					int preVal = readsCurrList.get(i).getReads().get(pos - readsCurrList.get(i).getStartAlignment() - 1);
					if (preVal == 0) 
					{
//						这样的话0,1,3三种情况都有了
	...
	private static byte[] EncodePbwtQual(ArrayList<ArrayList<Character>> vE)
	{
		...
		case 1:
			...
			for(int i = 0; i < keys.size()-8; i+=8){
				...
				byte by2 = (byte)(keys.get(i+2)%2 * 128 + keys.get(i+3)* 16 + keys.get(i+4)*2 + keys.get(i+5)/4 - 128);
...
//	之前的预处理我么可知，每一列必然有一个qual值，所以说不可能出现空的情况
	private static byte[] EncodePbwtSingleQual(ArrayList<Character> vE)
	{
		...
		if (pbwtQualProc.size()%8 == 0)
		{
			...
			{
				byte by1 = (byte)(pbwtQualProc.get(left)*32 + pbwtQualProc.get(left+1)*4 + pbwtQualProc.get(left+2)/2 -128);
				byte by2 = (byte)(pbwtQualProc.get(left+2)%2 * 128 + pbwtQualProc.get(left+3)* 16 + pbwtQualProc.get(left+4)*2 + pbwtQualProc.get(left+5)/4 - 128);
				byte by3 = (byte)(pbwtQualProc.get(left+5)%4 * 64 + pbwtQualProc.get(left+6)*8 + pbwtQualProc.get(left+7)-128 );
				pbwtQual.put(by1);
				pbwtQual.put(by2);
				pbwtQual.put(by3); 
			}
		...
		else
		{
			...
			case 2:
				...
				break;
			case 3:
				lastby = (byte)(pbwtQualProc.get(left)*32 + pbwtQualProc.get(left+1)*4 + pbwtQualProc.get(left+2)/2 -128);
			...
			case 5:
				lastby = (byte)(pbwtQualProc.get(left)*32 + pbwtQualProc.get(left+1)*4 + pbwtQualProc.get(left+2)/2 -128);
				lastby2 = (byte)(pbwtQualProc.get(left+2)%2 * 128 + pbwtQualProc.get(left+3)* 16 + pbwtQualProc.get(left+4)*2 - 128);
				pbwtQual.put(lastby);
				pbwtQual.put(lastby2);
				break;
			case 6:
				lastby = (byte)(pbwtQualProc.get(left)*32 + pbwtQualProc.get(left+1)*4 + pbwtQualProc.get(left+2)/2 -128);
				lastby2 = (byte)(pbwtQualProc.get(left+2)%2 * 128 + pbwtQualProc.get(left+3)* 16 + pbwtQualProc.get(left+4)*2 + pbwtQualProc.get(left+5)/4 - 128);
...
//		System.out.println(str[length-1].length());
	...
	private static int[] deStartPos(byte[] startPos)
	{
		...
		int len  = (startPos.length - 4)/deFlag;
		int deS[] = new int[len];
		deS[0] = deStart;
		...
		else if(deFlag == 2)
		{
			...
			{
				deS[index++] = ((startPos[i]&0xff)*256  + (startPos[i+1]&0xff)) 
						+ deS[index-2];
			}
		}
		else if(deFlag == 3)
		...
		else
		{
			...
			{
				deS[index++] = ((startPos[i]&0xff)*16777216 + (startPos[i+1]&0xff)*65536 + (startPos[i+3]&0xff)*256 + (startPos[i+4]&0xff))
						+ deS[index-2];
			}
		}
		return deS;
